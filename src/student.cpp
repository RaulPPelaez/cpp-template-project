...
std::tuple<std::string, int, double> generateStudentInfo() {
  static std::vector<std::string> names = {"Alice", "Bob", "Charlie", "David",
                                           "Eva"};
  ...
  return std::make_tuple(
      names[name_dist(gen)], age_dist(gen),
...
Student::Course::Course(const std::string &name, int y)
    : courseName(name), year(y) {}
...
void Student::enroll(const Course &course) { courses.push_back(course); }

void Student::printCourses() const {
  std::cout << "Enrolled courses:" << std::endl;
  for (const auto &course : courses) {
    std::cout << "  " << course.courseName << " (Year: " << course.year << ")"
              << std::endl;
  }
}
} // namespace student
